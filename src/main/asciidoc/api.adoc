...
Before we dive in, let's first look at a few key thing to keep in mind while using the API.
...
You may also need a little configuration in the client side. For example, in AngularJS, you will need to set the `withCredentials` flag of https://docs.angularjs.org/api/ng/service/$http#cross-site-request-forgery-xsrf-protection[$http] service, like this:
...
Refer the book [Spring Framework REST API Development - A Complete Blueprint](https://gum.co/exuo/associate) for details.
...
The book [Spring Framework REST API Development - A Complete Blueprint](https://gum.co/exuo/associate) covers it in details.
...
If some request data would not comply to the business rules, the API would respond with _400 bad request_, with a JSON body holding field-wise error details. For example, trying to [sign up](api/sign_up.html) with some invalid data could produce the following JSON data:
...
Obviously, the format of the error JSON, its field names and codes would vary among requests. Maintaining a documentation of each error for each request would be cumbersome. We may take that up sometime in future, but for now, the best way to know the format for any request would be to look at the actual response body by using a tool, such as the _google chrome developer tools_.

## JSON vulnerability

By default, Spring Lemon prefixes JSON responses with `)]}',\n`. This is done as a protection against the [JSON vulnerability](http://haacked.com/archive/2008/11/20/anatomy-of-a-subtle-json-vulnerability.aspx). Your client application would need to strip this `)]}',\n`. It's done automatically if you are using the [$http](https://docs.angularjs.org/api/ng/service/$http#json-vulnerability-protection) service of AngularJS.
...
Below is a sample of the common response headers that are received from the API by default.

```bash
C:\Users\skpat_000>curl -v http://localhost:8080/api/core/ping
* Hostname was NOT found in DNS cache
* Adding handle: conn: 0xfae710
* Adding handle: send: 0
* Adding handle: recv: 0
* Curl_addHandleToPipeline: length: 1
* - Conn 0 (0xfae710) send_pipe: 1, recv_pipe: 0
* Trying ::1...
...
< Set-Cookie: JSESSIONID=74F4A272BA49F9B0CFAF07EBEA534628; Path=/; HttpOnly
< Set-Cookie: XSRF-TOKEN=e06b98f4-d3b2-4f80-8435-b3c2f72d3182; Path=/
< X-Application-Context: application:dev
< Content-Length: 0
< Date: Fri, 24 Jul 2015 00:28:13 GMT
```
